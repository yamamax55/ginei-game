...
namespace Ginei
{
    ...
    public class MilitaryObserverOverlay : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // ESC は UIWindowStack 経由で「手前から閉じる」（M は従来どおり開閉トグル）。
            escWindowToken = UIWindowStack.Register(() => panel != null && panel.activeSelf, () => SetVisible(false), canvasSortingOrder, "軍編制");
        }
        ...
        private void BuildContentPanel(Transform parent)
        {
            ...
            WindowChrome.AddTitleBarLayout(frameRT, "軍 編制", () => SetVisible(false));
