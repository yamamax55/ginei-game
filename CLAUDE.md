...
| クラス | 責務 / 主なAPI |
|---|---|
| `StarSystem` | `[Serializable]` 星系ノード。`id`/`systemName`/`position`(Vector2)/`owner`(Faction)/`ownerData`(FactionData)。ctor`(id,name,pos,owner=帝国)`。 |
