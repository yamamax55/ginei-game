...
namespace Ginei
{
    ...
    /// キーは呼び出し側が <see cref="EntityKey.Of"/> で与える（64bit・切り詰めない）＝Core 純ロジック（UnityEngine 非依存）。
    /// ※セッション跨ぎの永続（セーブ）は別途＝instance id は不安定なので安定キーが要る（P2-b と合流）。test-first。
    /// </summary>
    public static class GrowthRegistry
    {
        private static readonly Dictionary<long, Growth> byPerson = new Dictionary<long, Growth>();

        /// <summary>id の成長を取得（無ければ <paramref name="archetype"/> で生成して登録）。</summary>
        public static Growth GetOrCreate(long personId, GrowthArchetype archetype)
        {
        ...
        public static Growth Get(long personId)
            => byPerson.TryGetValue(personId, out Growth g) ? g : null;

        /// <summary>id の成長が登録済みか。</summary>
        public static bool Has(long personId) => byPerson.ContainsKey(personId);

        /// <summary>経験を加算する（無ければ生成）。数式は <see cref="GrowthRules.GainExperience"/> へ委譲（二重実装しない）。</summary>
        public static void GainExperience(long personId, GrowthArchetype archetype, float amount, float dt)
