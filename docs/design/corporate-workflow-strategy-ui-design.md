...
- ~~**プレイヤーの立ち位置**：元首として全決裁するのか、一省庁の長として権限内だけ動かすのか~~ → **【解決済み】**：どちらでもなく**序列外の「目安箱＝越階の諫言回路」**とする（建白↑／諮問裁可↓／注入横の3動詞）。詳細は [[meyasubako-proposal-redesign-design]]（EPIC #MEYASU）。本 WF の状態機械を作り直さず、その上に「箱アクター・個人ごと信認・建白の伝播・LLM稟議生成」を足す発展形。
