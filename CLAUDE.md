...
| `GalaxyView` | 戦略マップのデモ可視化（MonoBehaviour）。`StrategySession` から銀河を復元（往復で状態保持）。星系/回廊/艦隊を実行時生成（ディスクスプライト・LineRenderer・TextMesh）。左ク=選択(Shift追加)／右ク=星系へ進軍 or 回廊上で停止保持（近い方=星系/回廊で曖昧解決）。Space/1/2/3 で速度。前線回廊は赤。**二層遷移(#586)**：回廊接触は即・実会戦へ強制遷移せず `BeginEngagements` で「交戦中の回廊」として固着表示（赤点滅・⚔交戦ラベル）。**交戦中の回廊をダブルクリック=潜行**（`TryDescend`→`BattleHandoff.Queue`→`Battle` シーン＝手動指揮）。放置は `autoResolveDelay`(銀河時間)後に `ResolveEncounters` で自動解決（Space停止で猶予停止＝intervene 可）。`Start` で `ApplyHandoffResult` 反映後、観ていない他戦線を `ResolveEncounters` で自動侵攻（統一時間の近似）。**惑星攻城(#131)**：デモは帝国星系に `Planet`(制空権持ち)を付与し、`Update` で `StrategyRules.TickSieges` を回す＝停泊した敵対艦隊が制空権制圧→侵略→占領。星系下に攻城状態(制空権残%/侵略%)を表示。攻城中の惑星を**ダブルクリックで戦術マップへ突入**(`TryDescendPlanet`→`BattleHandoff.QueuePlanetSiege`→Battle)。ハブ星系は `systemClickRadius` で星系クリックを回廊より優先（惑星へ進入可）。 |
| `BattleHandoff` | static シーン跨ぎの会戦受け渡し（C-3）。`StrengthScale`(40＝戦略兵力↔戦術 baseStrength)。`Pending`/`Resolved`/`factionA/B`/`strengthA/B`/`admiralA/B`/`fleetIdA/B`/`returnScene`/`sideAWon`/`survivorStrength`。`Queue(a,b,returnScene)`/`SetResult(aWon,survivors)`/`Clear()`。`BattleSetup` が Awake で `Pending` なら `SetupFromHandoff()` で2勢力を生成、`BattleManager` が決着で `WriteHandoffResultAndReturn`→残存兵力を `StrengthScale` で割り戻して `SetResult`→`returnScene` へ。**惑星攻城モード(#131)**：`IsPlanetSiege`/`planetSystemId`/`planetName`/`planetOwner`/`planetDefenseRatio`/`besiegerFaction`/`besiegerStrength`＋`QueuePlanetSiege(...)`＝戦略マップで惑星到着→ダブルクリックで戦術マップ突入（`BattleSetup.SetupPlanetSiege`／`BattleManager` は Backspace で `ReturnFromPlanetSiege`＝決着は書き戻さず戦略の `TickSieges` が継続）。 |
| `SiegeArena` | 惑星攻城の戦術アリーナ（#131 PB-1/PB-5・Battle シーン）。`BattleSetup.SetupPlanetSiege` が生成。中心に惑星＋**アルテミスの首飾り射程＝接近限界リング**(`approachRadius`)を描き、`LateUpdate` で旗艦がリング内（制空圏）へ入れないよう押し出す（`FleetMovement` 非改変＝BlackHole 方式）。中心の惑星に **制空権 `OrbitalDefense`(攻撃対象)** を付与し、攻城艦隊が砲撃で削る。**制空権が0＝ドメイン・ダウンで接近限界（押し出し・リング）を解除**し惑星へ近づけるようにする（HUDで通知）。制空権残量を惑星下に%表示（緑→赤・リング連動）。`Configure(approachRadius,planetScale,planetColor,label,planetOwner,orbitalDefenseMax,startRatio)`(`startRatio`＝戦略側の残り制空権で開始)。実行時生成のディスク/リング、`OnDestroy` で破棄。 |
| `OrbitalDefense` | 惑星攻城アリーナの**制空権（軌道防衛＝S-AV／アルテミスの首飾り）の被弾受け**(`IShipTarget`・#131)。`SiegeArena` が中心の惑星に付与。守備側勢力で `FleetRegistry` に登録され、攻城艦隊の通常索敵(`ShipCombat`)で自動的に標的になる＝**戦術マップ上で砲撃して削る**。`Initialize(owner,max,startRatio)`／`TakeDamage` で制空権を減算、0で `IsDomainDown`＝ドメイン・ダウン（レジストリから除外＋`OnDomainDown` イベント＝`SiegeArena` が接近限界を解く）。`Ratio`(0..1)。敵対判定・ダメージ式は持たず ShipCombat/FactionRelations に委譲。**戦術での削りは現状まだ戦略側 `Planet`/`TickSieges` へは書き戻さない**（書き戻しは別ステップ）。 |
