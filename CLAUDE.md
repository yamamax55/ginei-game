...
| `StrategyRules` | static 戦略ロジック。`FindEncounters`/`AnyEncounter`/`TryFindCollision`(回廊上で実際に接触したペア＝`FleetsCollided` の位置交差判定)/`IsFtlBlocked(map,corridor)`(端点所有者が敵対＝前線)/`ResolveOccupation`/`ResolveAllOccupations`(星系所有変動)/`ResolveCorridorBattle(aStr,dStr)`(攻撃側>守備側で勝利・残存=差・同数は守備)/`ResolveEncounters`/`ApplyBattleResult`/`ApplyHandoffResult(reg)`(実会戦の結果を盤面へ反映)。**戦闘式・接触判定をここに集約**。 |
| `GalaxyView` | 戦略マップのデモ可視化（MonoBehaviour）。`StrategySession` から銀河を復元（往復で状態保持）。星系/回廊/艦隊を実行時生成（ディスクスプライト・LineRenderer・TextMesh）。左ク=選択(Shift追加)／右ク=星系へ進軍 or 回廊上で停止保持（近い方=星系/回廊で曖昧解決）。Space/1/2/3 で速度。前線回廊は赤、保持中は「保持」表示。`Update` で `StrategyRules.TryFindCollision`→接触で `BattleHandoff.Queue`→`Battle` シーンへ。`Start` で `BattleHandoff.Resolved` なら `ApplyHandoffResult` で結果反映。 |
