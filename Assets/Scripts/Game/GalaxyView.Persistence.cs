...
namespace Ginei
{
    public partial class GalaxyView
    {
        ...
        public static void ResetCampaignStatics()
        {
            ...
            // 複数会戦の結果と援軍差し戻しは戦役固有。タイトル帰還・新規開始・ロード前に必ず捨て、
            // 前の戦役の決着が次の銀河へ適用されたり、旧艦隊IDを操作したりしないようにする（WIN-4）。
            ...
            FameRegistry.Clear();   // 実行時武名（ADM-3）も戦役固有＝持ち越さない（続きからは ProtagonistCareerSave.fame から復元）
