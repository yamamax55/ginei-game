...
{
    /// <summary>
    /// 労働オブザーバ（観測層・read-only）。<b>X キー</b>で開閉し、勢力ごとに所有惑星の**配線済みの労働・消費**を集約して
    /// 毎フレームライブダンプする：労働技能（<see cref="PopLaborTickRules.OverallSkill"/>＝就労シェア重み付き熟練度・#2034）／
    /// 賃金指数（<see cref="Province.wageIndex"/>＝労働逼迫×技能・#1969）／就業率（<see cref="OccupationRules.EmploymentRate"/>・#110）／
    /// 生活水準（<see cref="Province.livingStandard"/>＝労働→所得→消費の帰結・#181）／飢餓（<see cref="Province.foodShortage"/>＝必需不足・#2042）。
    /// `GalaxyView.RunAnnualLifecycleTick`（年次）が技能形成・職業配分・賃金・消費を回す＝そのループの帰結を映す。
    /// 観測専用ゆえ既存フィールドのみ読む。操作はさせない＝**状態は変えない**。`HelpOverlay` と同型の自動生成（Strategy/Battle）。
    /// </summary>
    public class LaborObserverOverlay : MonoBehaviour
    {
        [Header("外観")]
        public int canvasSortingOrder = 1101;
        ...
        [Tooltip("飢餓とみなす必需不足の閾値（foodShortage がこれ以上の惑星を飢餓として数える）")]
        public float famineThreshold = 0.2f;
        ...
        private static void TryCreate(Scene scene)
        {
            ...
            if (Object.FindAnyObjectByType<LaborObserverOverlay>() != null) return;
            new GameObject("LaborObserverOverlay").AddComponent<LaborObserverOverlay>();
        }
        ...
        private void Awake()
        {
            BuildUI();
            SetVisible(false);
            escWindowToken = UIWindowStack.Register(() => panel != null && panel.activeSelf, () => SetVisible(false), canvasSortingOrder, "労働");
        }
        ...
        private void Update()
        {
            if (GameInput.WasPressed(GameAction.労働観測切替))
                Toggle();
        ...
        private string BuildDump()
        {
            ...
            sb.Append("<b>労働オブザーバ</b>　労働技能・賃金・就業率・生活水準・飢餓　(X で閉じる)\n");
            ...
            if (c == null || c.states == null || c.states.Count == 0 || map == null || provinces == null)
            {
                ...
                sb.Append("戦略マップ（GalaxyView）を起動すると、各勢力の労働・生活水準がライブ表示されます。");
                return sb.ToString();
            }
            ...
            sb.Append("\n<color=#6f8a9a>※ 労働→所得→消費→生活水準→支持のループ。教育#155が技能を、技能と労働逼迫が賃金#1969を、生活水準が民心#113を動かす。</color>");
            return sb.ToString();
        }
        ...
        {
            float skillW = 0f, livingW = 0f, wageW = 0f, empW = 0f, popSum = 0f;
            float worstFood = 0f; int famine = 0, ownedCount = 0;
            ...
            foreach (var kv in provinces)
            {
                ...
                float pw = Mathf.Max(0f, prov.population);
                popSum += pw;
                skillW  += PopLaborTickRules.OverallSkill(prov) * pw;
                livingW += prov.livingStandard * pw;
                wageW   += prov.wageIndex * pw;
                empW    += OccupationRules.EmploymentRate(prov) * pw;

                if (prov.foodShortage > worstFood) worstFood = prov.foodShortage;
                if (prov.foodShortage >= famineThreshold) famine++;
            }

            sb.Append('\n').Append("<color=#e7e0b0>◤ ").Append(fac).Append("</color>");
            sb.Append("　<color=#6f8a9a>(惑星 ").Append(ownedCount).Append(")</color>\n");
            if (ownedCount == 0 || popSum <= 0f)
            {
                sb.Append("  <color=#9aa7b2>（所有惑星なし）</color>\n");
                return;
            }

            float skill  = skillW / popSum;
            float living = livingW / popSum;
            float wage   = wageW / popSum;
            float emp    = empW / popSum;

            AppendBar(sb, "  労働技能", Mathf.Clamp01(skill), "#7fd4ff");
            AppendBar(sb, "  就業率",   Mathf.Clamp01(emp), emp < 0.7f ? "#ffd28a" : "#a0e0a0");
            sb.Append("  <color=#9fb0c0>賃金指数</color> ＝ <color=#ffe08a>").Append(wage.ToString("0.00"))
              .Append("</color>　<color=#6f8a9a>(1.0＝基準・>1＝労働逼迫で上昇)</color>\n");
            AppendBar(sb, "  生活水準", Mathf.Clamp01(living), living < 0.4f ? "#ff9a8a" : "#a0e0a0");

            if (famine > 0)
                sb.Append("  <color=#ff7a6a>⚠ 飢餓 ").Append(famine).Append(" 惑星（最悪 必需不足 ")
                  .Append((worstFood * 100f).ToString("0")).Append("%）</color>\n");
        }
        ...
        private void BuildUI()
        {
            ...
            overlayRoot = new GameObject("LaborObserverCanvas");
        ...
        private void BuildContentPanel(Transform parent)
        {
            ...
            WindowChrome.AddTitleBarLayout(frameRT, "労働", () => SetVisible(false));
