...
| `SiegeArena` | 惑星攻城の戦術アリーナ（#131 PB-1/PB-5・Battle シーン）。`BattleSetup.SetupPlanetSiege` が生成。中心に惑星＋**アルテミスの首飾り射程＝接近限界リング**(`approachRadius`)を描き、`LateUpdate` で旗艦がリング内（制空圏）へ入れないよう押し出す（`FleetMovement` 非改変＝BlackHole 方式）。`Configure(approachRadius,planetScale,planetColor,label)`。実行時生成のディスク/リング、`OnDestroy` で破棄。 |
