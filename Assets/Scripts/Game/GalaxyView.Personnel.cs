...
namespace Ginei
{
    public partial class GalaxyView
    {
        ...
        private void RunAnnualLifecycleTick()
        {
            ...
            // 生産連鎖(#2091)/BOM(#2098)/SCM(#2105)は Tick改善P1 で月割り stagger へ移設（RunMonthlyCampaignTick・各年1回）。
            // 外交(#2119)は Tick改善P2 で四半期へ移設（応答性）。
            ...
            // 財産税（#163/#917 配線）：司令の固定資産税＋富裕税（平均超過に累進）を徴収し国庫へ＝資産→税→国庫の環。格差を縮める。
            RunAssetTaxTick();
        ...
        private void RunFinancialMarkToMarket()
        {
            ...
            for (int i = 0; i < holdings.Count; i++)
            {
                ...
                float bondFactor = 1f + (bondPrice - 1f) * 0.1f * MonthDt;       // 国債価格で月次ドリフト
