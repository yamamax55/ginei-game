...
namespace Ginei.Tests
{
    ...
    public class PartyRulesTests
    {
        ...
        {
            var bureaucrat = new Office(1, "事務次官", OfficeScope.国家, OfficeDomain.内政) { requiredTier = 9 };
            var minister = new Office(2, "内務大臣", OfficeScope.国家, OfficeDomain.内政) { requiredTier = 11 };
            ...
            var careerOfficial = new Person(1, "官僚", Faction.同盟, PersonRole.文民) { rankTier = 11, isPolitician = false };
            Assert.IsFalse(OfficeRules.CanHold(careerOfficial, minister));
            Assert.IsTrue(OfficeRules.CanHold(careerOfficial, bureaucrat));

            var politician = new Person(2, "政治家", Faction.同盟, PersonRole.文民) { rankTier = 11, isPolitician = true };
