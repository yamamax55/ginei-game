...
| `BattleManager` | Battle シーンに1つ | 勝敗判定（`checkInterval` 秒ごと、`EvaluateVictory`）・戦績記録。**勝利条件**は `ScenarioData.victoryCondition` に従う：殲滅/時間防衛(`objectiveFaction` が `timeLimit` 秒生存)/旗艦撃破(`targetAdmiral` 撃破)/護衛(`targetAdmiral` を守り切る)。**どの条件でも片陣営の旗艦全滅は常に終了条件**。生存数は `FleetRegistry.GetFlagships()`（退却・破棄・配下艦は数えない）。経過時間 `battleElapsed` は timeScale 追従（ポーズで停止・倍速で加速）。勝者・勝因(`victoryReason`)を `GameSettings` に記録し `ResultManager` が表示。開始時隻数は全 Start 完了後の最初の Update で記録。決着で timeScale=0→Result へ遷移。Rキーでリスタート。 |
