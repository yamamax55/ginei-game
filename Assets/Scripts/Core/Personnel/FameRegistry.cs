...
namespace Ginei
{
    ...
    /// 会戦の実効武名は「max(基準, 実行時)」で読む（基準非破壊＝実効値パターン）。キーは呼び出し側が <see cref="EntityKey.Of"/> で与える＝
    /// Core 純ロジック（UnityEngine 非依存）。永続は安定キーで別途（主人公分は ProtagonistCareerSave.fame）。test-first。
    /// </summary>
    public static class FameRegistry
    {
        private static readonly Dictionary<long, int> byPerson = new Dictionary<long, int>();

        /// <summary>id の武名（無ければ0）。</summary>
        public static int Get(long personId) => byPerson.TryGetValue(personId, out int v) ? v : 0;

        /// <summary>id の武名を設定（0..100 クランプ）。</summary>
        public static void Set(long personId, int fame)
