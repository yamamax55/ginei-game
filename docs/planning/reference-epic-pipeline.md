...
- **プレフィックス一意**：3〜4文字の略号（ALM/SAW…）。`docs/planning/roadmap.md` と `docs/reference/*-reference-design.md` を grep して衝突がないことを確認。
...
### Step 3 — 設計書ドラフト
`docs/reference/<slug>-reference-design.md`。章立ては固定（SAW/ALM に倣う）：
...
```
本文の型：先頭に `親EPIC #<n> ／ 設計：docs/reference/<slug>-reference-design.md §2` ＋ `## 狙い` `## 接続先` `## 完了条件`。
...
3. `docs/planning/roadmap.md` §5-2 へ1行追記：`- **<作品> 参考：#<EPIC>（EPIC）＋XXX-1〜N（#a〜#b）** ＝<要約・三本柱・不採用・着手順>`。
