...
| `OrderOfBattlePanel` | Battle シーン | **O キーで開閉する編制管理UI（#147・`OrderOfBattle`/`FleetRoster` ビューア＋組み替え）**。プレイヤー勢力の梯団ツリー（軍集団 ⊃ 軍団 ⊃ 第N艦隊＋司令の階級名）を表示し、**艦隊の［移動］→軍団の［ここへ］で別軍団へ移す（中身流動）**＝`OrderOfBattle.AttachFleet`＋該当 `FleetStrength.corpsName`/`armyGroupName` を更新してHUDへ即反映。未編入艦隊も一覧。数値ロジックは持たず static 窓口を読むだけ。**表示中 `Time.timeScale=0` でポーズ**（`PauseManager` は `OrderOfBattlePanel.IsOpen` の間ポーズ入力を譲る）。`HelpOverlay` と同じ `RuntimeInitializeOnLoadMethod` で Battle に自動生成。`IsOpen`/`Toggle`/`Close`。 |
