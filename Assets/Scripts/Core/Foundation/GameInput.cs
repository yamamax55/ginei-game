...
namespace Ginei
{
    ...
    public static class GameInput
    {
        ...
        private static readonly InputBinding[] table =
        {
            ...
            // 法令オブザーバ（法の支配/法治・治安〔犯罪→秩序→抑圧〕を観測＝戦略/会戦で開閉）。
            new InputBinding(GameAction.法令観測切替,     Key.L,         InputContext.共通),
            // 教育オブザーバ（教育チェーン 幼→小→中→高→上級 と人材供給を観測＝戦略/会戦で開閉）。
            ...
            // 第2弾観測オーバーレイ（単一文字キーは A〜Z 使い切ったため Alt＋文字。上メニューからも開ける）。
            new InputBinding(GameAction.財政観測切替,     Key.F,         InputContext.共通, alt: true),
            new InputBinding(GameAction.生産観測切替,     Key.P,         InputContext.共通, alt: true),
            new InputBinding(GameAction.政府観測切替,     Key.G,         InputContext.共通, alt: true),
            new InputBinding(GameAction.造船観測切替,     Key.B,         InputContext.共通, alt: true),
            new InputBinding(GameAction.人物動態観測切替, Key.L,         InputContext.共通, alt: true),
            ...
            new InputBinding(GameAction.執務机切替,       Key.J,         InputContext.共通, alt: true),
            ...
            new InputBinding(GameAction.攻城戦術切替,   Key.T,         InputContext.会戦),
            // ミニマップ（俯瞰・クリックジャンプ #84）。M は軍観測（共通）が使用済みのため会戦専用の C を割当。
            new InputBinding(GameAction.ミニマップ切替, Key.C,         InputContext.会戦),
        ...
        private static readonly Dictionary<GameAction, string> descriptions = new Dictionary<GameAction, string>
        {
            ...
            { GameAction.財政観測切替,   "財政オブザーバを開く / 閉じる（歳入歳出・国債/債務スパイラル・予算配分・金利）" },
            { GameAction.生産観測切替,   "生産・流通オブザーバを開く / 閉じる（生産チェーン・消費財BOM在庫）" },
            { GameAction.政府観測切替,   "政府オブザーバを開く / 閉じる（要職任命・省庁ツリー・首班・文民統制）" },
            ...
            { GameAction.稟議観測切替,     "稟議オブザーバを開く / 閉じる（進行中の稟議の起案者・決裁者・状態・出自）" },
            { GameAction.執務机切替,       "主人公の執務机を開く / 閉じる（階級・配属・主命・武勲・人脈・一代記＝軍人立志伝の一人称シェル）" },
            ...
            { GameAction.攻城戦術切替,   "攻城戦術を切替（強襲↔包囲）＝惑星攻城中のみ" },
            { GameAction.ミニマップ切替, "ミニマップを表示 / 非表示（クリックで現場へジャンプ）" },
