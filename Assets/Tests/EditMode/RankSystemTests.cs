...
namespace Ginei.Tests
{
    ...
    /// 完全12段ラダーに合わせ tier: 7准将/8少将/9中将/10大将/12元帥（11＝上級大将は同盟型で欠番）で検証する。
    /// </summary>
    public class RankSystemTests
    {
        // 同盟型（11＝上級大将が欠番）の階級表を持つ FactionData を作る。
        ...
        public void NextRankTier_ReturnsSmallestHigher()
        {
            ...
            Assert.AreEqual(8, RankSystem.NextRankTier(f, 7)); // 准将→少将
            Assert.AreEqual(12, RankSystem.NextRankTier(f, 10)); // 大将→元帥（11欠番を飛ばす）
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            Assert.AreEqual(12, RankSystem.NextRankTier(f, 12));
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            Assert.AreEqual(9, RankSystem.PreviousRankTier(f, 10)); // 大将→中将
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            Assert.AreEqual(7, RankSystem.PreviousRankTier(f, 7));
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            // 11（上級大将）は同盟型に無い → 直近下位の 10（大将）へ。
            Assert.AreEqual(10, RankSystem.ResolveTier(f, 11));
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            // 3 は最下位(7)より下 → 直近下位が無いので直近上位の 7 へ。
            Assert.AreEqual(7, RankSystem.ResolveTier(f, 3));
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            Assert.AreEqual("大将", RankSystem.ResolveRankName(f, 10));
            Assert.AreEqual("元帥", RankSystem.ResolveRankName(f, 12));
        }
        ...
        {
            var f = MakeAllianceLikeFaction();
            // 11（上級大将）は同盟型に無い → 直近下位 10（大将）の名称。
            Assert.AreEqual("大将", RankSystem.ResolveRankName(f, 11));
        }
        ...
        {
            Assert.AreEqual("中将", RankSystem.DefaultRankName(9));
            Assert.AreEqual("上級大将", RankSystem.DefaultRankName(11));
            Assert.AreEqual("元帥", RankSystem.DefaultRankName(12));
        }
        ...
        public void DefaultRankName_OutOfRange_IsEmpty()
        {
            ...
            Assert.AreEqual("", RankSystem.DefaultRankName(6));  // 准将(7)の直下＝将官フォールバック外
            Assert.AreEqual("", RankSystem.DefaultRankName(13));
        }
        ...
        public void ResolveRankNameOrDefault_NullFaction_UsesDefaultLadder()
        {
            ...
            Assert.AreEqual("中将", RankSystem.ResolveRankNameOrDefault(null, 9));
        }
        ...
        public void ResolveRankNameOrDefault_FactionTablePreferred()
        {
            var f = MakeAllianceLikeFaction();
            // 勢力の階級表に該当があればそれを使う。
            Assert.AreEqual("中将", RankSystem.ResolveRankNameOrDefault(f, 9));
            // 同盟型に無い 11 は表側で直近下位 10（大将）へ丸め → 既定の「上級大将」ではなく「大将」。
            Assert.AreEqual("大将", RankSystem.ResolveRankNameOrDefault(f, 11));
        }
        ...
        // 帝国型（11＝上級大将を含む完全ラダー）。同盟型との非対称検証に使う。
        private FactionData MakeImperialLikeFaction()
        {
            ...
            f.ranks = new List<FactionData.RankEntry>
            {
                ...
                new FactionData.RankEntry(9, "中将"),
                new FactionData.RankEntry(10, "大将"),
                new FactionData.RankEntry(11, "上級大将"),
                new FactionData.RankEntry(12, "元帥"),
            };
            return f;
        }
        ...
        public void ResolveTier_NullRanksList_ReturnsSame()
        {
            var f = ScriptableObject.CreateInstance<FactionData>();
            f.ranks = null;
            Assert.AreEqual(9, RankSystem.ResolveTier(f, 9));
        }

        // (2) ResolveTier: 全ランクより上の tier は直近下位（最上位 tier=12）へスナップ。
        // 仕様：完全一致無し→直近下位優先。99 は 12 へ丸まるべき。
        [Test]
        public void ResolveTier_AboveAll_SnapsToTop()
        {
            var f = MakeAllianceLikeFaction();
            Assert.AreEqual(12, RankSystem.ResolveTier(f, 99));
        }

        // (3) ResolveTier: 両側に候補がある欠番は「直近下位」を優先する非対称性。
        // 同盟型(…10,12…)で tier 11 は下10/上12 の両方が在るが、仕様は下位優先＝10。
        [Test]
        public void ResolveTier_BetweenTwo_PrefersLowerNotHigher()
        {
            var f = MakeAllianceLikeFaction();
            Assert.AreEqual(10, RankSystem.ResolveTier(f, 11));
            // 帝国型なら 11 は実在 → 11 のまま（非対称の対照）。
            Assert.AreEqual(11, RankSystem.ResolveTier(MakeImperialLikeFaction(), 11));
        }
        ...
        public void NextRankTier_SkipsNullEntries()
        {
            ...
            // 7 の次は（null を飛ばして）10。
            Assert.AreEqual(10, RankSystem.NextRankTier(f, 7));
            // 10 が最上位（null 以外で上が無い）→ 据え置き。
        ...
        public void PreviousRankTier_SkipsNullEntries()
        {
            ...
            Assert.AreEqual(7, RankSystem.PreviousRankTier(f, 10));
            // 7 が最下位 → 据え置き。
            Assert.AreEqual(7, RankSystem.PreviousRankTier(f, 7));
        }
        ...
        public void DefaultRankName_NegativeAndBoundary_IsEmpty()
        {
            ...
            Assert.AreEqual("", RankSystem.DefaultRankName(6));   // 7 の直下
            Assert.AreEqual("准将", RankSystem.DefaultRankName(7)); // 下端
            Assert.AreEqual("元帥", RankSystem.DefaultRankName(12)); // 上端
            Assert.AreEqual("", RankSystem.DefaultRankName(int.MaxValue));
        }

        // (10) ResolveRankNameOrDefault: 階級表が空でも既定ラダーへフォールバックする。
        // 空 ranks → ResolveTier は tier 据え置き → GetRankName は "" → DefaultRankName(9)="中将"。
        [Test]
        public void ResolveRankNameOrDefault_EmptyTable_FallsBackToDefaultLadder()
        {
            ...
            f.ranks = new List<FactionData.RankEntry>();
            Assert.AreEqual("中将", RankSystem.ResolveRankNameOrDefault(f, 9));
            // 既定ラダー（将官7-12）にも無い tier は空文字。
            Assert.AreEqual("", RankSystem.ResolveRankNameOrDefault(f, 6));
        }
        ...
        public void ResolveRankName_EmptyNameEntry_ReturnsEmpty()
        {
            ...
            f.ranks = new List<FactionData.RankEntry>
            {
                new FactionData.RankEntry(10, ""),
            };
            Assert.AreEqual("", RankSystem.ResolveRankName(f, 10));
            // ただし OrDefault は空名を「引けなかった」とみなし既定ラダーへ → 大将。
            Assert.AreEqual("大将", RankSystem.ResolveRankNameOrDefault(f, 10));
        }
    }
}
