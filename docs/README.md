...
## ディレクトリ構成（Obsidian Vault）
`docs/` を Obsidian の Vault として開くと、各エリアの索引（MOC）から辿れる。各docには `type`/`tags` の frontmatter を付与済み。
...
## 2026-09-09 ローカル開発の終了記録

- [開発引継ぎ・統合状況・次回検証](ops/development-handoff-2026-09-09.md) — 本日終了。最新の通常占領・駐留艦隊仕様と、実機確認済み／未確認を区別。

## Claude Code・ChatGPTの開発分担

[開発手順と各段階の完了条件](ops/claude-chatgpt-development-workflow.md)を次回開発から適用する。
