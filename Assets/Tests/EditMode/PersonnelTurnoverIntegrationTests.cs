...
namespace Ginei.Tests
{
    ...
    public class PersonnelTurnoverIntegrationTests
    {
        ...
        public void Vacancy_FilledBySeniorEligibleSuccessor_OnDeath()
        {
            ...
            var a = Officer(1, Faction.帝国, 11); // 現職（大将級以上）
            var b = Officer(2, Faction.帝国, 10); // 後任有資格
            var c = Officer(3, Faction.帝国, 7); // 階級不足＝資格なし
        ...
        public void Vacancy_StaysEmpty_WhenNoEligibleSuccessor()
        {
            ...
            { militaryOnly = true, requiredTier = 10 };
            var a = Officer(1, Faction.同盟, 10);
            GovernmentRegistry.TryAppoint(Faction.同盟, office, a);
            a.deathYear = Y;

            var c = Officer(3, Faction.同盟, 7); // 階級不足のみ
