...
namespace Ginei
{
    /// <summary>国家予算の歳出分野（国家予算の基盤）。既存番号を保つため教育は末尾。配分の読み書きは <see cref="BudgetRules"/>。</summary>
    public enum BudgetCategory { 軍事, 建艦, 内政, 社会保障, 研究, 外交, 教育 }

    /// <summary>
    /// 国家予算＝歳出を分野ごとに配分した計画（国家予算の基盤）。<see cref="FiscalState"/> の単一歳出 baseExpenditure を
    /// 分野（軍事/建艦/内政/社会保障/研究/外交/教育）へ分解する＝歳入（国庫/税収）をどこへ振り向けるかという高位の決断の器。
    ...
    public class NationalBudget
    {
        ...
        [Tooltip("教育（在学容量・学校品質 #998。/ターン）")] public float education;

        public NationalBudget() { }

        public NationalBudget(float military, float shipbuilding, float administration,
                              float welfare, float research, float diplomacy, float education = 0f)
        {
            ...
            this.education = Mathf.Max(0f, education);
        }
    }
}
