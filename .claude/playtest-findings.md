...
- [x] [Core] flow25（並列8・2026-06-19）：思想・宗教・文化テーマ8本（地球教型）＝ReligiousCultRules（秘密教団の信徒獲得と政治介入）/StateReligionRules（国教と政教の緊張）/HereticSuppressionRules（異端弾圧と殉教の逆説）/ZealotryRules（狂信の戦力化と暴走）/IndoctrinationRules（教化と思想注入）/CulturalHegemonyRules（グラムシ的文化覇権と同意調達）/ApocalypticMovementRules（終末思想と認知的不協和）/SchismRules（教団分裂と分派）を新規実装（各EditModeテスト付き・TestHarness 7773 green）。
- [x] [Core] flow26（並列8・2026-06-19）：傭兵・補助兵力・封建軍テーマ8本＝MercenaryCompanyRules（傭兵団＝金の切れ目が縁の切れ目）/AuxiliaryForcesRules（補助部隊＝現地兵の反乱リスク）/ForeignLegionRules（外人部隊＝祖国なき者の忠誠）/ContractSoldierRules（契約兵＝職業兵士の労働市場）/VassalLevyRules（封臣の軍役＝リップシュタット貴族連合型）/MilitiaRules（民兵＝郷土愛と地の利）/WarbandRules（武装集団＝戦利品で結束）/FreebooterRules（無頼の冒険者＝宇宙海賊の汚れ仕事）を新規実装（各EditModeテスト付き・TestHarness 7853 green）。
- [x] [Core] flow27（並列8・2026-06-19）：戦傷・医療・戦争の人的コストテーマ8本＝CasualtyEvacuationRules（傷病兵後送のゴールデンタイム）/FieldMedicineRules（野戦医療と治療成績）/CombatTraumaRules（長期PTSDと回復）/VeteranReintegrationRules（退役兵の社会復帰と政治化）/WoundedRecoveryRules（負傷療養と戦列復帰）/MedicalTriageRules（限られた医療資源の優先配分）/ShellShockRules（戦闘中の即時的崩壊とパニック伝染）/WarDeadRules（戦没者の弔いと厭戦）を新規実装（各EditModeテスト付き・TestHarness 7947 green）。
- [x] [Core] flow28（並列8・2026-06-19）：地上戦・惑星制圧テーマ8本＝GroundAssaultRules（軌道からの降下強襲）/UrbanWarfareRules（市街戦＝攻者三倍と立体戦）/ArmoredWarfareRules（機甲戦の突破と機動包囲）/InfantryCombatRules（歩兵の火力運動と分隊戦術）/PlanetaryInvasionRules（惑星規模の侵攻＝軌道制圧前提）/BeachheadRules（橋頭堡の確保と拡張）/CombinedGroundAirRules（近接航空支援と制空）/SpecialForcesRaidRules（特殊部隊の奇襲と非対称効果）を新規実装（各EditModeテスト付き・TestHarness 8023 green）。
- [x] [Core] flow29（並列8・2026-06-19）：指揮通信・C3I・電子戦テーマ8本＝CommandNetworkRules（C3I網の堅牢性と分散冗長）/SignalCorpsRules（通信兵科の回線確立と帯域）/ElectronicCountermeasuresRules（ECM/ECCMの応酬と電子的優越）/DataLinkRules（戦術データ共有と協調交戦）/CommandPostRules（司令部の処理能力と斬首脆弱性）/NetworkCentricWarfareRules（ネットワーク中心戦の戦力相乗）/CommunicationRelayRules（星間長距離通信の中継網）/SituationalAwarenessRules（状況認識三層とOODA）を新規実装（各EditModeテスト付き・TestHarness 8545 green）。
- [x] [Core] flow30（並列8・2026-06-19）：宇宙艦隊大戦略・海軍ドクトリンテーマ8本＝DecisiveBattleDoctrineRules（艦隊決戦主義の妥当性）/CommerceProtectionRules（勢力規模の通商保護戦略）/NavalBaseRules（軍事根拠地の戦略価値）/PowerProjectionRules（遠方への戦力投射と過剰投射）/StrategicChokepointRules（戦略的隘路の地政学的決定性）/ForwardDeploymentRules（前方展開と抑止・孤立リスク）/MaritimeStrategyRules（マハン的制宙戦略）/NavalArmsRaceRules（建艦競争のエスカレーションと安定）を新規実装（各EditModeテスト付き・TestHarness 9031 green）。
